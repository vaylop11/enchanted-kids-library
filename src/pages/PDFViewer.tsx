...
import { ArrowLeft, ArrowRight, Loader2, Plus, Download, XCircle } from 'lucide-react';
...
const PDFViewer = () => {
  ...
  const { data: supabasePdf, refetch: refetchSupabasePdf } = useQuery({
    ...
    queryFn: () => getSupabasePDFById(pdfId!),
    enabled: !!pdfId,
    retry: false
  });
  ...
  const { data: supabaseChats, refetch: refetchSupabaseChats } = useQuery({
    ...
    queryFn: () => getSupabaseChatsByPdfId(pdfId!),
    enabled: !!pdfId && isSupabasePDF,
    retry: false,
    meta: {
      onSuccess: (data: SupabaseChat[]) => {
  ...
  const handleDeleteMessage = async (messageId: string) => {
    try {
      ...
      toast.error(language === 'ar' ? 'حدث خطأ أثناء حذف الرسالة' : 'Error occurred while deleting message');
    } finally {
        ...
        <section className="lg:w-2/3 flex flex-col">
          <div className="relative">
            ...
            <div className="absolute top-2 left-2 bg-secondary/80 text-secondary-foreground rounded-md p-2 shadow-md">
              <p className="text-sm">
                {language === 'ar' ? `صفحة ${pageNumber} من ${numPages}` : `Page ${pageNumber} of ${numPages}`}
              </p>
            </div>
          </div>
