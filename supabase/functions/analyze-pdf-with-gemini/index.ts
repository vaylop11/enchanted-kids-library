...
serve(async (req) => {
  ...
  try {
    ...
    if (!pdfText || !userQuestion) {
      ...
      throw new Error('Missing required parameters: PDF text or question');
    }
    ...
    const maxTextLength = 30000; // Set limit for Gemini context window
    
    if (textLength > maxTextLength) {
      console.log(`PDF text exceeds ${maxTextLength} characters, using chunking strategy`);
      ...
      console.log(`Chunked text length: ${processedText.length} characters`);
    }
    ...
    // Validate data structure to ensure we have what we need
    ...
    console.log(`Successfully generated response of ${generatedText.length} characters`);
